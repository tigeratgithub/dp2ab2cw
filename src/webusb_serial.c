...
#define WEBUSB_URL_STRINGS                                                     \
  'e', 'x', 'a', 'm', 'p', 'l', 'e', '.', 't', 'i', 'n', 'y', 'u', 's', 'b',   \
      '.', 'o', 'r', 'g', '/', 'w', 'e', 'b', 'u', 's', 'b', '-', 's', 'e',    \
      'r', 'i', 'a', 'l', '/', 'i', 'n', 'd', 'e', 'x', '.', 'h', 't', 'm',    \
      'l'
...
const uint8_t USBD_WinUSBDescriptorSetDescriptor[USBD_WINUSB_DESC_SET_LEN] = {
    ...
    0x04,
    0x00, /*  wDescriptorType: MS_OS_20_FEATURE_REG_PROPERTY: 0x04 (Table 9) */
    ...
    0x00, 0x50, 0x00, /* wPropertyDataLength  */
    /* bPropertyData: “{975F44D9-0D08-43FD-8B3E-127CA8AFFF9D}” */
    '{', 0x00, '9', 0x00, 'd', 0x00, '7', 0x00, 'd', 0x00, 'e', 0x00, 'b', 0x00,
    'b', 0x00, 'c', 0x00, '-', 0x00, 'c', 0x00, '8', 0x00, '5', 0x00, 'd', 0x00,
    '-', 0x00, '1', 0x00, '1', 0x00, 'd', 0x00, '1', 0x00, '-', 0x00, '9', 0x00,
    'e', 0x00, 'b', 0x00, '4', 0x00, '-', 0x00, '0', 0x00, '0', 0x00, '6', 0x00,
    '0', 0x00, '0', 0x00, '8', 0x00, 'c', 0x00, '3', 0x00, 'a', 0x00, '1', 0x00,
    '9', 0x00, 'a', 0x00, '}', 0x00, 0x00, 0x00, 0x00, 0x00};

const uint8_t USBD_WebUSBURLDescriptor[URL_DESCRIPTOR_LENGTH] = {
    URL_DESCRIPTOR_LENGTH, WEBUSB_URL_TYPE, WEBUSB_URL_SCHEME_HTTPS,
    WEBUSB_URL_STRINGS};
...
struct usb_bos_descriptor bos_desc = {.string =
                                          USBD_BinaryObjectStoreDescriptor,
                                      .string_len = USBD_BOS_WTOTALLENGTH};

static const uint8_t device_descriptor[] = {USB_DEVICE_DESCRIPTOR_INIT(
    USB_2_1, 0xEF, 0x02, 0x01, 0xcafe, USBD_PID, 0x0002, 0x01)};

static const uint8_t config_descriptor_hs[] = {
    USB_CONFIG_DESCRIPTOR_INIT(USB_CONFIG_SIZE, ITF_NUM_TOTAL, 0x01,
                               USB_CONFIG_BUS_POWERED, USBD_MAX_POWER),
    CDC_ACM_DESCRIPTOR_INIT(ITF_NUM_CDC, CDC_INT_EP, CDC_OUT_EP, CDC_IN_EP,
                            USB_BULK_EP_MPS_HS, 0x04),
    USB_INTERFACE_DESCRIPTOR_INIT(ITF_NUM_VENDOR, 0x00, 0x02,
                                  USB_DEVICE_CLASS_VEND_SPECIFIC, 0xff, 0x00,
                                  0x05),
    USB_ENDPOINT_DESCRIPTOR_INIT(WEBUSB_IN_EP, USB_ENDPOINT_TYPE_BULK,
                                 USB_BULK_EP_MPS_HS, 0x00),
    USB_ENDPOINT_DESCRIPTOR_INIT(WEBUSB_OUT_EP, USB_ENDPOINT_TYPE_BULK,
                                 USB_BULK_EP_MPS_HS, 0x00),
};
...
static const uint8_t config_descriptor_fs[] = {
    USB_CONFIG_DESCRIPTOR_INIT(USB_CONFIG_SIZE, ITF_NUM_TOTAL, 0x01,
                               USB_CONFIG_BUS_POWERED, USBD_MAX_POWER),
    CDC_ACM_DESCRIPTOR_INIT(ITF_NUM_CDC, CDC_INT_EP, CDC_OUT_EP, CDC_IN_EP,
                            USB_BULK_EP_MPS_FS, 0x04),
    USB_INTERFACE_DESCRIPTOR_INIT(ITF_NUM_VENDOR, 0x00, 0x02,
                                  USB_DEVICE_CLASS_VEND_SPECIFIC, 0xff, 0x00,
                                  0x05),
    USB_ENDPOINT_DESCRIPTOR_INIT(WEBUSB_IN_EP, USB_ENDPOINT_TYPE_BULK,
                                 USB_BULK_EP_MPS_FS, 0x00),
    USB_ENDPOINT_DESCRIPTOR_INIT(WEBUSB_OUT_EP, USB_ENDPOINT_TYPE_BULK,
                                 USB_BULK_EP_MPS_FS, 0x00),
};
...
static const uint8_t other_speed_config_descriptor_fs[] = {
    USB_OTHER_SPEED_CONFIG_DESCRIPTOR_INIT(USB_CONFIG_SIZE, ITF_NUM_TOTAL, 0x01,
...
static const char *const string_descriptors[] = {
    ...
    "TomcatIO",                       /* Manufacturer */
    "Rotary Encoder Pulse Converter", /* Product */
...
static void usbd_event_handler(uint8_t busid, uint8_t event) {
  ...
  case USBD_EVENT_CONFIGURED:
    ...
    usbd_ep_start_read(busid, CDC_OUT_EP, read_buffer_1,
                       usbd_get_ep_mps(busid, CDC_OUT_EP));
    usbd_ep_start_read(busid, WEBUSB_OUT_EP, read_buffer_2,
                       usbd_get_ep_mps(busid, WEBUSB_OUT_EP));
    break;
...
void usbd_webusb_out(uint8_t busid, uint8_t ep, uint32_t nbytes) {
  usbd_ep_start_write(busid, WEBUSB_IN_EP, read_buffer_2,
                      nbytes); /* echo back */
...
struct usbd_endpoint webusb_out_ep = {.ep_addr = WEBUSB_OUT_EP,
                                      .ep_cb = usbd_webusb_out};

struct usbd_endpoint webusb_in_ep = {.ep_addr = WEBUSB_IN_EP,
...
static int
vendor_class_interface_request_handler(uint8_t busid,
